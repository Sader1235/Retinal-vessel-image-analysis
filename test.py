...
def train(trainloader, model, optimizer, epoch, dir):
    ...
    for batch_idx, (inputs_x, targets_x, name_x) in enumerate(trainloader):
        # try:
        ...
        # errors.update(error_value, inputs_x.size(0))
...
def validate(valloader, model, epoch, dir):
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets, names) in enumerate(valloader):
            ...
            # errors.update(error_value, inputs.size(0))
            #save output
            # if random.random() > 0.:
...
def visulize(a, b, c, d, epoch, dir, Threshod):
    ...
    if epoch % 1 == 0:
        for batch_num in range(len(a)):
            ...
            cv2.imwrite(dir + str(name_save) + '_Inp.png', 255 * inputs_save)
            cv2.imwrite(dir + str(name_save) + '_Out.png', outputs_save)
            
            cv2.imwrite(dir + str(name_save) + '_Tar.png', targets_save)
